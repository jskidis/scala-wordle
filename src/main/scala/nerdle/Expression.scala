...
case class OneOperatorExpression(operand1: Int, operator: NerdleOperator, operand2: Int) extends IntExpression {
  lazy val treeRep: OperatorExpr = OperatorExpr(IntValueExpr(operand1), operator, IntValueExpr(operand2))

  override def value: Int = treeRep.value
  override def isValid: Boolean = treeRep.isValid
  override def toString: String = treeRep.toString
}
