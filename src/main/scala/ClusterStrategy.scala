...
trait ClusterStrategy extends SolveStrategy with WordColorPatternGenerator with WordPatternMatcher {
  case class WordClusterCount(word: XordleWord, clusterCount: Int)
  ...
  def reduceWordSet(wordSet: WordSet, currentGuess: String, colorPattern: List[BlockColor]): WordSet = {
    ...
    wordSet.filter { w: XordleWord => doesWordMatch(w.string, wordPattern) && w.string != currentGuess }
  }
  ...
  def generateNextGuess(remainingWords: WordSet): (String, String) = {
    ...
    val wordClusters: List[WordClusterCount] = remainingWords.toList.map { potentialAnswer: XordleWord =>
      val clusterCount = remainingWords.map{ word : XordleWord => generateWordColorPattern(potentialAnswer, word)}.size
  ...
  def sortWordCluster(wc1: WordClusterCount, wc2: WordClusterCount): Boolean = (wc1.word, wc2.word) match {
    ...
    case (ww1: XordleWord, ww2: XordleWord) =>
