...
trait BasicLenAndCharValidator  {
  ...
  def invalidCharMsg(validChars: Seq[Char]): String = s"Input may only contain ${validChars.mkString(", ")}"
  def wasResultMsg: String = "That was a word, not a result"

  def validateInput(input: String, len: Int, validChars: Seq[Char]): Option[String] = {
  ...
  def validateGuess(input: String, len: Int, validChars: Seq[Char], resultChars: Seq[Char]): Option[String] = {
